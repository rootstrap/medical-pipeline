...
spark_task = LivyOperator(
    ...
    conf={
            ...
            "spark.kubernetes.container.image" : "rootstrap/spark-py:uml_concepts1.8",
            "spark.kubernetes.authenticate.driver.serviceAccountName" : "spark",
            "spark.kubernetes.namespace" : "airflow",
            "spark.kubernetes.driverEnv.INPUT_DIR" : "/data/output/" + str(datetime.today().strftime('%d%m%Y')),
            "spark.kubernetes.driverEnv.OUTPUT_DIR" : "/data/output/" + str(datetime.today().strftime('%d%m%Y')),
            "spark.kubernetes.executorEnv.INPUT_DIR" : "/data/results/" + str(datetime.today().strftime('%d%m%Y')),
            "spark.kubernetes.executorEnv.OUTPUT_DIR" : "/data/results/" + str(datetime.today().strftime('%d%m%Y')),
